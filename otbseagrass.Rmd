...
sgtrnd <- reactive({
  ...
  trndest <- covest %>% 
    ...
    group_by(Savspecies, Transect) %>% 
    nest %>% 
    mutate(
      trnd = purrr::map(data, function(data){
        
        mod <- lm(bbest ~ Date, data)
        
        slo <- coefficients(mod)[2]
  ...
  p <- ggplot(trndest, aes(x = Savspecies, y = Transect, fill = slo)) + 
    geom_tile(colour = 'grey') +
    geom_text(aes(label = pval)) + 
    scale_x_discrete(expand = c(0, 0)) + 
    scale_y_discrete(expand = c(0, 0)) + 
    scale_fill_gradient2('chg/yr', low = 'lightblue', mid = 'white', high = 'lightgreen') + 
...
wqtrnd <- reactive({
  ...
  p <- ggplot(trndest, aes(x = var, y = factor(epchc_station), fill = slo)) + 
    geom_tile(colour = 'grey') +
    geom_text(aes(label = pval)) + 
    scale_x_discrete(expand = c(0, 0)) + 
    scale_y_discrete(expand = c(0, 0)) + 
...
```{r}
output$wqtrnd <- renderPlotly(wqtrnd())
plotlyOutput('wqtrnd', height = 600, width = 600)
```
...
```{r}
output$sgtrnd <- renderPlotly(sgtrnd())
plotlyOutput('sgtrnd', height = 600, width = 600)
```