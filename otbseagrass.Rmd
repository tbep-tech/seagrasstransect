...
names(algnms) <- algnms
wqalgnms <- c(list('Total nitrogen' = 'tn', 'Chorophyll-a' = 'chla', 'Secchi' = 'sd_raw_m', 'Salinity' = 'Sal_Mid_ppth'), as.list(algnms))
...
mltcmpplo2 <- reactive({
  ...
  mxv <- toplo %>% 
    mutate(
      ...
      covmx = cov_ave + cov_brs,
      epcmx = epc_ave + epc_brs
    ) %>% 
    gather('var', 'val', algmx, covmx, epcmx) %>% 
    group_by(var) %>% 
  ...
  p1 <- plot_ly(toplo, x = ~yr, y = ~epc_ave, mode = 'markers', type = 'scatter', error_y = list(array = ~epc_brs, width = 0), color = I('lightblue')) %>%  
    add_trace(mode = 'markers', marker = mrk, showlegend = F) %>% 
    layout(
      yaxis = list(title = wqvar3),# range = c(0, mxv[['epcmx']])),
      xaxis = list(title = 'Year', gridcolor = '#FFFFFF')
      )
  p2 <- plot_ly(toplo, x = ~yr, y = ~alg_ave, mode = 'markers', type = 'scatter', error_y = list(array = ~alg_brs, width = 0), color = I('orange')) %>%  
    add_trace(mode = 'markers', marker = mrk, showlegend = F) %>% 
    layout(
      yaxis = list(title = algvar3),# range = c(0, mxv[['algmx']])),
      xaxis = list(title = 'Year', gridcolor = '#FFFFFF')
    )
  p3 <- plot_ly(toplo, x = ~yr, y = ~cov_ave, mode = 'markers', type = 'scatter', error_y = list(array = ~cov_brs, width = 0), color = I('lightgreen')) %>%
    add_trace(mode = 'markers', marker = mrk, showlegend = F) %>% 
    layout(
      yaxis = list(title = sgvar3), #range = c(0, mxv[['covmx']])),
      xaxis = list(title = 'Year', gridcolor = '#FFFFFF')
    )
...
mltcmpplo3 <- reactive({
  ...
  p1 <- plot_ly(toplo, type = 'scatter', mode = 'markers') %>%
  ...
  p2 <- plot_ly(toplo, type = 'scatter', mode = 'markers') %>%
  ...
  p3 <- plot_ly(toplo, type = 'scatter', mode = 'markers') %>%
...
fillCol(
  flex = c(NA, NA, NA),
  column(12,
    column(4, selectInput('wqvar3', 'Select water quality:', choices = wqalgnms[!names(wqalgnms) %in% algnms])),
