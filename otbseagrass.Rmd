...
trndmapsel <- reactive({
  ...
  m1 <- m %>% 
    clearMarkers() %>% 
    clearControls() %>% 
    addMarkers(data = sgtrnd1[sgtrnd1$cols == 'green', ], ~LONG_DD, ~LAT_DD, icon = makeIcon('www/upsg.png', 15, 15), label = ~paste0('Transect: ', TRAN_ID, ', Tau: ', tau, ', p ', pval)) %>% 
    addMarkers(data = sgtrnd1[sgtrnd1$cols == 'blue', ], ~LONG_DD, ~LAT_DD, icon = makeIcon('www/dnsg.png', 15, 15), label = ~paste0('Transect: ', TRAN_ID, ', Tau: ', tau, ', p ', pval)) %>% 
    addMarkers(data = sgtrnd1[sgtrnd1$pval != 'ns', ], ~LONG_DD, ~LAT_DD, icon = makeIcon('www/sigsg.png', 15, 15), label = ~paste0('Transect: ', TRAN_ID, ', Tau: ', tau, ', p ', pval)) %>% 
    addMarkers(data = wqalgtrnd1[wqalgtrnd1$cols == 'green', ], ~Longitude, ~Latitude, icon = makeIcon('www/upwq.png', 15, 15), label = ~paste0('Station: ', epchc_station, ', Tau: ', tau, ', p ', pval)) %>% 
    addMarkers(data = wqalgtrnd1[wqalgtrnd1$cols == 'blue', ], ~Longitude, ~Latitude, icon = makeIcon('www/dnwq.png', 15, 15), label = ~paste0('Station: ', epchc_station, ', Tau: ', tau, ', p ', pval)) %>% 
    addMarkers(data = wqalgtrnd1[wqalgtrnd1$pval != 'ns', ], ~Longitude, ~Latitude, icon = makeIcon('www/sigwq.png', 15, 15), label = ~paste0('Station: ', epchc_station, ', Tau: ', tau, ', p ', pval))
  
  
  m2 <- m %>% 
    clearMarkers() %>% 
    clearControls() %>% 
    addMarkers(data = sgtrnd2[sgtrnd2$cols == 'green', ], ~LONG_DD, ~LAT_DD, icon = makeIcon('www/upsg.png', 15, 15), label = ~paste0('Transect: ', TRAN_ID, ', Tau: ', tau, ', p ', pval)) %>% 
    addMarkers(data = sgtrnd2[sgtrnd2$cols == 'blue', ], ~LONG_DD, ~LAT_DD, icon = makeIcon('www/dnsg.png', 15, 15), label = ~paste0('Transect: ', TRAN_ID, ', Tau: ', tau, ', p ', pval)) %>% 
    addMarkers(data = sgtrnd2[sgtrnd2$pval != 'ns', ], ~LONG_DD, ~LAT_DD, icon = makeIcon('www/sigsg.png', 15, 15), label = ~paste0('Transect: ', TRAN_ID, ', Tau: ', tau, ', p ', pval)) %>% 
    addMarkers(data = wqalgtrnd2[wqalgtrnd2$cols == 'green', ], ~Longitude, ~Latitude, icon = makeIcon('www/upwq.png', 15, 15), label = ~paste0('Station: ', epchc_station, ', Tau: ', tau, ', p ', pval)) %>% 
    addMarkers(data = wqalgtrnd2[wqalgtrnd2$cols == 'blue', ], ~Longitude, ~Latitude, icon = makeIcon('www/dnwq.png', 15, 15), label = ~paste0('Station: ', epchc_station, ', Tau: ', tau, ', p ', pval)) %>% 
    addMarkers(data = wqalgtrnd2[wqalgtrnd2$pval != 'ns', ], ~Longitude, ~Latitude, icon = makeIcon('www/sigwq.png', 15, 15), label = ~paste0('Station: ', epchc_station, ', Tau: ', tau, ', p ', pval))
  ...
  out <- allann %>% 
    ...
    mutate(
      data = purrr::map(data, function(x){
        ...
        los <- try({t.test(x$val)$conf.int[1]})
        his <- try({t.test(x$val)$conf.int[2]})
        
        if(inherits(ave, 'try-error'))
          ave <- NA
          
        if(inherits(los, 'try-error'))
          los <- NA
          
        if(inherits(his, 'try-error'))
          his <- NA
        
        sumout <- data.frame(ave = ave, los = los, his = his)
...
mltcmpplo1 <- reactive({
  ...
  tocmb <- c(unlist(wqalgnms), savnms)
  trnds <- crossing(cmb1 = tocmb, cmb2 = tocmb) %>% 
    ...
    mutate(
      ...
      cmb1 = factor(cmb1, levels = rev(tocmb), labels = rev(c(names(wqalgnms), savnms))), 
      cmb2 = factor(cmb2, levels = rev(tocmb), labels = rev(c(names(wqalgnms), savnms)))
    ) 
  ...
  p <- ggplot(toplo, aes(x = cmb1, y = cmb2, fill = corv)) + 
    ...
    scale_x_discrete(expand = c(0, 0)) + 
    scale_y_discrete('EPCHC station', expand = c(0, 0)) + 
    scale_fill_gradient2('Spearman rho', low = 'lightblue', mid = 'white', high = 'lightgreen', midpoint = 0) + 
    theme(
...
mltcmpplo2 <- reactive({
  ...
  # input
  wqvar2 <- input$wqvar2
  algvar2 <- input$algvar2
  sgvar2 <- input$sgvar2
  mltcmp1 <- mltcmp1()
  
  toplo <- mltcmp1 %>% 
    filter(var %in% c(wqvar2, algvar2, sgvar2)) %>% 
    mutate(brs = (his - los) / 2) %>% 
    select(-los, -his)
  
  toplo1 <- toplo %>% filter(dat %in% 'epc')
  toplo2 <- toplo %>% filter(dat %in% 'alg')
  toplo3 <- toplo %>% filter(dat %in% 'cov')
  
  p1 <- plot_ly(toplo1, x = ~yr, y = ~ave, mode = 'markers', type = 'scatter', error_y = list(array = ~brs)) %>%  
    layout(
      yaxis = list(title = wqvar2),
      xaxis = list(title = NA, gridcolor = '#FFFFFF')
      )
  p2 <- plot_ly(toplo2, x = ~yr, y = ~ave, mode = 'markers', type = 'scatter', error_y = list(array = ~brs)) %>%  
    layout(
      yaxis = list(title = algvar2),
      xaxis = list(title = NA, gridcolor = '#FFFFFF')
    )
  p3 <- plot_ly(toplo3, x = ~yr, y = ~ave, mode = 'markers', type = 'scatter', error_y = list(array = ~brs)) %>%  
    layout(
      yaxis = list(title = sgvar2),
      xaxis = list(title = NA, gridcolor = '#FFFFFF')
    )
  
  out <- subplot(p1, p2, p3, nrows = 3, shareX = T, titleY = T) %>% 
    layout(showlegend = F)
...
fillCol(
  ...
  column(12,
    column(6, 
      ...
      selectInput('wqalgvar1', 'Select water quality/phyto map 1:', choices = wqalgnms)
    ), 
    column(6, 
      selectInput('sgvar2', 'Select seagrass map 2:', choices = savnms, selected = 'Halodule'), 
      selectInput('wqalgvar2', 'Select water quality/phyto map 2:', choices = wqalgnms, selected = 'chla')
    ),
  ),
...
fillCol(
  flex = c(NA, 1),
  column(12,
    column(4, selectInput('wqvar2', 'Select water quality:', choices = wqalgnms[!names(wqalgnms) %in% algnms])),
    column(4, selectInput('algvar2', 'Select phyto:', choices = algnms)),
    column(4, selectInput('sgvar2', 'Select seagrass:', choices = savnms))
    ), 
  plotlyOutput('mltcmpplo2', height = "700px", width = "900px")
)
```
